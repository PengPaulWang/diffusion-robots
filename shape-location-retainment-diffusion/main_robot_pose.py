...
def train_video_predictor(cfg):
    ...
    train_dataset = FrameSet(frames=frames, crop_size=crop_size, robot_poses=robot_poses)
    train_loader = DataLoader(train_dataset, batch_size=1, num_workers=4, shuffle=True)
...
def train_video_predictor_ckpt(cfg):
    ...
    diffusion = ConditionalDiffusion(model, training_target='noise', noise_schedule='cosine',
                                     timesteps=cfg.diffusion_timesteps)
# /home/staff/peng/myGoogleDrive/SinFusion/lightning_logs/waffle_pi_ii/waffle_pi_ii_robot_pose_video_model_0_predictor/checkpoints/last.ckpt
    checkpoint_path = 'lightning_logs/waffle_pi_ii/waffle_pi_ii_robot_pose_video_model_0_predictor/checkpoints/last.ckpt'
