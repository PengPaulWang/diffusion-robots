...
"""
def sample_pose_diff_old(condition_frame, start_frame_index = None, current_frame_index = None, robot_poses = None, frame_diff = 1):
...
def sample_pose_diff(condition_frame, start_frame_index = None, current_frame_index = None, frame_diff = 1):
    frame_to_generate_id = start_frame_index + current_frame_index
    conditioned_frame_id = frame_to_generate_id - frame_diff
    ...
    return conditioned_frame_id, frame_to_generate_id
...
def generate_video(cfg):
    ...
    # robot_poses = read_pose_from_file(f'./robot_data/robot_pose.txt')
    # robot_poses = read_pose_from_file(f'./robot_data/robot_pose.txt')
    ...
    for frame in range(1, cfg.output_video_len + 1):
        ...
        condition_frame_id, frame_to_generated_id = sample_pose_diff(samples[-1], start_frame_index = cfg.start_frame_index, current_frame_index=frame, frame_diff=cfg.frame_diff)
        ...
        next_frame = predictor_model.sample(condition=samples[-1], pose_diff=None, frame_diff=cfg.frame_diff, mask=mask)
