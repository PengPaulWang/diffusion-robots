...
from models.modules import ConvNextBlock, SinusoidalPosEmb, ConvNextBlockMaskEmbeddingDecoder, ConvNextBlockMaskEmbeddingEncoder
import pysnooper
...
class NextNet(nn.Module):
    ...
    """
    def __init__(self, in_channels=3, out_channels=3, depth=16, filters_per_layer=64, frame_conditioned=False, mask_channels=None):
        """
        Args:
            in_channels (int):
...
# ---
        ...
        self.layers.append(ConvNextBlockMaskEmbeddingDecoder(in_channels, dims[0], emb_dim=emb_dim, norm=False, mask_channels=mask_channels))

        for i in range(1, math.ceil(self.depth / 2)):
            self.layers.append(ConvNextBlockMaskEmbeddingDecoder(dims[i - 1], dims[i], emb_dim=emb_dim,  norm=True, mask_channels=mask_channels))
        for i in range(math.ceil(self.depth / 2), depth):
            self.layers.append(ConvNextBlockMaskEmbeddingDecoder(2 * dims[i - 1], dims[i], emb_dim=emb_dim, norm=True, mask_channels=mask_channels)) # x and x from 'encoding' procedure concatenated, hence why 2*dims[i-1]
# ---
...
# ---
    ...
    # @pysnooper.snoop()
    def forward(self, x, t, robot_pose=None, frame_diff=None, mask=None):
