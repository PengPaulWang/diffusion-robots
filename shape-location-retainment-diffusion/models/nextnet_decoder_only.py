...
# ---
        ...
        self.layers.append(ConvNextBlockMaskEmbeddingEncoder(in_channels, dims[0], emb_dim=emb_dim, norm=False))

        for i in range(1, math.ceil(self.depth / 2)):
            self.layers.append(ConvNextBlockMaskEmbeddingEncoder(dims[i - 1], dims[i], emb_dim=emb_dim,  norm=True))
