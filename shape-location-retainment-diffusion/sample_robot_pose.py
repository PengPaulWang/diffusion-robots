...
#         condition_mask=mask_transform(masks[condition_frame_id,:,:,:]).to(predictor_model.device).unsqueeze(0)
#         to_generate_mask=mask_transform(masks[frame_to_generated_id,:,:,:]).to(predictor_model.device).unsqueeze(0)
#         mask = torch.cat([condition_mask, to_generate_mask], dim=1)
      

        # print(f'====predictor_model = {predictor_model.device} ====')
        next_frame = predictor_model.sample(condition=samples[-1], pose_diff = pose_diff, frame_diff=cfg.frame_diff)

        # Correct the sampled frame
        noisy_next_frame = noise_img(next_frame, projector_model, correction_t)
        corrected_next_frame = projector_model.sample(custom_initial_img=noisy_next_frame, custom_timesteps=correction_t) # needs look into
