...
class FrameSet(Dataset):
    ...
    """
    def __init__(self, frames, masks, crop_size=None, robot_poses=None, dataset_size=5000):
        """
        Args:
    ...
    def __getitem__(self, item):
        ...
        condition_mask = self.transform(self.masks[frame_idx - frame_diff])
        condition_idx = frame_idx - frame_diff

        random.seed(seed)
        torch.random.manual_seed(seed)
        frame = self.transform(self.frames[frame_idx])
        mask = self.transform(self.masks[frame_idx])
        
        pose_diff = self.robot_poses[frame_idx] - self.robot_poses[condition_idx]
        pose_diff = torch.from_numpy(pose_diff).float()
