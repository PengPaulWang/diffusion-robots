...
class Diffusion(LightningModule):
    def __init__(self, model, channels=3, timesteps=1000,
        ...
        assert self.training_target in ['x0', 'noise']  # when 'x0' is to recontruct the image, when 'noise' is to recontruct noise
        ...
        posterior_variance = betas * (1. - alphas_hat_prev) / (1. - alphas_hat)  # A little bit weird
    ...
    # The denoising processing
    # x_0 = 1./sqrt(alphas_hat)*x_t - (sqrt(1-alphas_hat)/sqrt(alphas_hat))* epsilon
    # Refer to equation (1) in the SinFusion paper
    def predict_start_from_noise(self, x_t, t, noise):
        return self.sqrt_recip_alphas_hat[t] * x_t - self.sqrt_recipm1_alphas_hat[t] * noise

    # https://lilianweng.github.io/posts/2021-07-11-diffusion-models/
    # Refer to the definition of miu_tild_t
    ...
    def p_mean_variance(self, x, t, clip_denoised, mask=None):
        batch_size = x.shape[0]
        t_tensor = torch.full((batch_size,), t, dtype=torch.int64, device=self.device)

        if self.training_target == 'x0':
            x_recon = self.model(x, t_tensor, mask=mask)
        else:
            x_recon = self.predict_start_from_noise(x, t=t, noise=self.model(x, t_tensor, mask=mask))
    ...
    @torch.no_grad()
    def p_sample(self, x, t, clip_denoised=True, mask=None):
        model_mean, _, model_log_variance = self.p_mean_variance(x=x, t=t, clip_denoised=clip_denoised, mask=mask)
        noise = torch.randn_like(x) if t > 0 else torch.zeros_like(x)  # no noise when t == 0
        return model_mean + noise * (0.5 * model_log_variance).exp()

    @torch.no_grad()
    def sample(self, image_size=(32, 32), batch_size=16, custom_initial_img=None, custom_timesteps=None, mask=None):
        """
        ...
        Args:
            ...
            custom_timesteps (int):
                ...
                This can be used for denoising of partially noised images.
        """
        # if only one size provided, it will be converted into a tuple (x,x)
        ...
        for t in reversed(range(0, timesteps)):
            img = self.p_sample(img, t, mask=mask)
        return img
...
### These are for training
    ...
    def forward(self, x, *args, **kwargs):
        ...
        t_tensor = torch.full((batch_size, ), t, dtype=torch.int64, device=self.device)
        if self.training_target == 'x0':
            x0_recon = self.model(x_noisy, t_tensor, mask=mask)  
            return F.mse_loss(x, x0_recon)
        else:
            noise_recon = self.model(x_noisy, t_tensor, mask=mask)
