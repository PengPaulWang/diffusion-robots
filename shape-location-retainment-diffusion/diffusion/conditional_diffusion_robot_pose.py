...
class ConditionalDiffusion(LightningModule):
    ...
    def p_mean_variance(self, x, t, clip_denoised, condition_x=None, pose_diff=None, frame_diff=None):
        ...
        if self.training_target == 'x0':
            ...
            x_recon = self.predict_start_from_noise(x, t=t, noise=self.model(torch.cat([condition_x, x], dim=1),
                                                                             noise_level, pose_diff,fd_tensor))
    ...
    def p_sample(self, x, t, clip_denoised=True, condition_x=None, pose_diff = None, frame_diff=None):
        model_mean, model_log_variance = self.p_mean_variance(
            x=x, t=t, clip_denoised=clip_denoised, condition_x=condition_x, pose_diff=pose_diff, frame_diff=frame_diff)
        b, c, h, w = x.shape
        if t > 0:
    ...
    @torch.no_grad()
    def sample(self, condition, pose_diff = None, frame_diff=None):
        """
        ...
        for i in reversed(range(0, self.num_timesteps)):  #num_timesteps = 1000
            img = self.p_sample(img, i, condition_x=condition, pose_diff = pose_diff, frame_diff=frame_diff)
        return img
    ...
    def forward(self, x_in, noise=None):
        ...
        recon = self.model(
            torch.cat([x_in['CONDITION_IMG'], x_noisy], dim=1), continuous_sqrt_alpha_hat.view(-1), x_in.get('POSE'), x_in.get('FRAME'))
