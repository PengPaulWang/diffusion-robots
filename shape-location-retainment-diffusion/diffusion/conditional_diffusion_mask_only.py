...
class ConditionalDiffusion(LightningModule):
    ...
    def p_mean_variance(self, x, t, clip_denoised, condition_x=None, pose_diff=None, frame_diff=None, mask=None):
        ...
        if self.training_target == 'x0':
            ...
            x_recon = self.predict_start_from_noise(x, t=t, noise=self.model(torch.cat([condition_x, x], dim=1),
                                                                             noise_level, pose_diff,fd_tensor, mask=mask))
            #noise_level is generated by the model
        if clip_denoised:
            x_recon.clamp_(-1., 1.)
    ...
    def p_sample(self, x, t, clip_denoised=True, condition_x=None, pose_diff = None, frame_diff=None, mask=None):
        model_mean, model_log_variance = self.p_mean_variance(
            x=x, t=t, clip_denoised=clip_denoised, condition_x=condition_x, pose_diff=pose_diff, frame_diff=frame_diff, mask=mask)
        b, c, h, w = x.shape
        if t > 0:
    ...
    @torch.no_grad()
    def sample(self, condition, pose_diff = None, frame_diff=None, mask=None):
        """
        ...
        for i in reversed(range(0, self.num_timesteps)):  #num_timesteps = 1000
            img = self.p_sample(img, i, condition_x=condition, pose_diff = pose_diff, frame_diff=frame_diff, mask=mask)
        return img
    ...
    def forward(self, x_in, noise=None):
        ...
        # x_pose = x_in['POSE']
        x_mask = x_in['MASK']
        x_condition_mask = x_in['CONDITION_MASK']
        # print(f'-----------forward x_pose = {x_pose} ----------------')
        b = x_start.shape[0]
        ...
        mask = torch.cat([x_condition_mask, x_mask], dim=1)
        recon = self.model(
            torch.cat([x_in['CONDITION_IMG'], x_noisy], dim=1), continuous_sqrt_alpha_hat.view(-1), robot_pose=None, frame_diff=x_in.get('FRAME'), mask=mask)
